...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($date && $formationId) {
        $formationDate = new FormationDate(null, $date, $formationId);
        $formationDateBusiness->create($formationDate);
        header("Location: listFormationDate.php?message=created");
        exit;
    }
}
?>
...
<head>
    <meta charset="UTF-8">
    <title>Ajouter une Date de Formation</title>
    <style>
        body {
...
<div class="container">
    <h1>➕ Ajouter une Date de Formation</h1>
    ...
    <form method="POST">
        ...
        <select name="formationId" id="formationId" required>
            ...
            <?php foreach ($formations as $formation): ?>
                <option value="<?= $formation->id ?>">Formation #<?= $formation->id ?> - <?= htmlspecialchars($formation->mode) ?></option>
            <?php endforeach; ?>
        </select>
