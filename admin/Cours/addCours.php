...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $coursBusiness->create($cours);
    header("Location: listCours.php?message=added");
    exit;
}
?>
    ...
    <div class="max-w-3xl mx-auto bg-white shadow-md rounded-lg p-6">
        ...
        <form method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="3" class="w-full border rounded p-2"></textarea>
            </div>
            ...
            <div>
                ...
                <input type="text" name="audience" id="audience" class="w-full border rounded p-2">
            </div>

            <div>
                <label for="duration" class="block font-medium">Durée (heures)</label>
                <input type="number" name="duration" id="duration" min="0" step="1" class="w-full border rounded p-2">
            </div>

            <div class="flex items-center space-x-2">
                <input type="checkbox" name="testIncluded" id="testIncluded">
            ...
            <div>
                ...
                <textarea name="testContent" id="testContent" rows="2" class="w-full border rounded p-2"></textarea>
            </div>

            <div>
                <label for="logo" class="block font-medium">Logo (image)</label>
            ...
            <div>
                ...
                <select name="sujetId" id="sujetId" required class="w-full border rounded p-2">
                    ...
                    <?php foreach ($sujets as $sujet): ?>
                        <option value="<?= $sujet->id ?>"><?= htmlspecialchars($sujet->name) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
